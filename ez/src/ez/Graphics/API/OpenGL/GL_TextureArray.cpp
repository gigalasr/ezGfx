...
namespace ez {
    ...
    GL_TextureArray::~GL_TextureArray() { glDeleteTextures(1, &m_texture); }

    void GL_TextureArray::bind() { glBindTexture(GL_TEXTURE_2D_ARRAY, m_texture); }

    uint32_t GL_TextureArray::get_max_layers() { return m_max_layers; }
    ...
    void GL_TextureArray::erase(Layer layer) { m_free_layers.push(layer); }

    // HTexture OpenGLAPI::CreateTexture(uint32_t width, uint32_t height,
    // TextureFormat format, TextureFilter filter) { 	GLenum
    // internalNativeFormat =
    // texture_format_to_internal_native(format); 	GLenum nativeFormat =
    // texture_format_to_native(format); 	GLenum nativeFilter =
    // texture_filter_to_native(filter);
    ...
    // tex.Format, GL_UNSIGNED_BYTE, data);
    // }

    // void OpenGLAPI::UploadTexture(HTextureArray hTexture, uint32_t layer,
    // void* data) { 	GLTextureArray tex = m_TextureArrays[(int)hTexture];
    //	glBindTexture(GL_TEXTURE_2D_ARRAY, tex.Id);
    //	glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, layer, tex.width,
    // tex.height, 1, tex.Format, GL_UNSIGNED_BYTE, data);
    // }
}  // namespace ez